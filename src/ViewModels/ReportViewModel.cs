...
public sealed class ReportViewModel : ObservableObject
{
    ...
    public ReportViewModel(MainViewModel shell)
    {
        ...
        DeleteAllLogsCommand = new RelayCommand(DeleteAllLogs, () => HasLogs);
        Refresh();
    }
    ...
    public RelayCommand DeleteAllLogsCommand { get; }

    /// <summary>Mirrors MainViewModel.WriteLog so the Report tab can toggle it directly.</summary>
    public bool WriteLog
    {
        get => _shell.WriteLog;
        set
        {
            _shell.WriteLog = value;
            OnPropertyChanged();
        }
    }
    ...
    /// <summary>Label shown below the toggle, explaining the current state.</summary>
    public string WriteLogHint => WriteLog
        ? "Log files are saved to disk after each run."
        : "Log files are not saved. Enable \u201cSave log files\u201d to keep a record of every run.";
    ...
    private void Refresh()
    {
        ...
        OnPropertyChanged(nameof(WriteLogHint));
        DeleteAllLogsCommand.RaiseCanExecuteChanged();
    }
