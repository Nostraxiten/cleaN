...
| **Informe y logs** | Un log en texto plano de cada limpieza: cada archivo eliminado y el espacio total recuperado. El guardado de logs está **desactivado por defecto**; actívalo con *Guardar logs* en la cabecera o en la pestaña de Informe. Desde esa misma pestaña puedes borrarlos todos de una vez |
...
- **Cada ejecución puede quedar registrada**, para que siempre puedas auditar lo que pasó. El guardado de logs es opcional y puede activarse o borrarse en bloque desde la pestaña de Informe.
...
- **El guardado de logs está desactivado por defecto.** Una instalación nueva ya no escribe
  ningún archivo en `%LOCALAPPDATA%\cleaN\logs`. Actívalo con la casilla *Guardar logs* de
  la barra superior o desde la pestaña *Informe y logs*.
- **Botón «Eliminar todos los logs».** La pestaña de Informe incluye ahora un botón que borra
  todos los archivos de log guardados con un solo clic. El botón está deshabilitado cuando
  no hay logs que eliminar.
